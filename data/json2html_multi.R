# library("rjson") # not should be used ?
library("RJSONIO")

code <- "" 

path <- "GitHub/webpage/data/data"
jList <- list.files(path, pattern="json")

m <- 1
while(m <= length(jList)){
file <- strsplit(jList[m], ".json")[[1]]
...
################################################
path.o <- paste("GitHub/webpage/docs/article/", htmlf, sep="") #docs or data
